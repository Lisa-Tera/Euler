# -*- coding: utf-8 -*-
"""
Created on Tue Mar  3 20:29:34 2020
...
# 출발지, 도착지를 받았을 때 계산하기 쉽게 테이블을 변환하는 함수
...
def calc_table(short):
    for n in range(2,len(short)):
        for i in range(len(short)-n):
            temp = []
            for j in range(n):
                if j ==(n-1): temp.append(short[i+j+1][i])
                else: temp.append(short[i+j+1][i]+short[i+n][i+j+1])
...
while True:
    try:
        ...
        if i[0] < 0 or i[1] >= len(short) :
            raise ValueError
        break
    except ValueError: print('위치 인덱스를 잘못 입력했습니다.')

if index[i[0]] == index[i[1]]: distance = 0
else:
